...
resp_test!(pfadd_duplicate_no_change, c, {
    let k = common::key("hll");
...
resp_test!(pfcount_missing_zero, c, {
    assert_eq!(c.pfcount(&[common::key("hll")]).await.unwrap(), 0);
});

resp_test!(pfcount_union, c, {
    let k1 = common::key("hll1");
    let k2 = common::key("hll2");
...
resp_test!(pf_wrong_type_errors, c, {
