...
//! Cluster-mode integration tests for the parity features (batch options,
...
use glide::{
    Batch, BatchOptions, CustomCommand, GlideClusterClient, GlideClusterClientConfiguration,
    PubSubMessageKind, Route, ScriptingCommands, SortedSetCommands,
};
...
async fn cluster_exec_with_options() {
    ...
    let mut b = Batch::new(false);
    b.set(&k, "1").incr(&k).get(&k);
    let opts = BatchOptions::new()
        .with_timeout(Duration::from_secs(5))
        .with_retry_server_error(true);
    let results = c.exec_with_options(&b, true, None, &opts).await.unwrap();
    ...
    let mut tx = Batch::new(true);
    tx.set(&k2, "5").incr(&k2);
    let res2 = c
        .exec_with_options(&tx, true, None, &BatchOptions::new())
        .await
        .unwrap();
...
async fn cluster_runtime_subscribe_receive() {
    ...
    let n = publisher.publish(&chan, "hello").await.unwrap();
