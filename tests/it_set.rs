...
resp_test!(scard_missing_zero, c, {
...
resp_test!(smembers_missing_empty, c, {
    assert!(c.smembers(common::key("s")).await.unwrap().is_empty());
});

resp_test!(sismember, c, {
    let k = common::key("s");
...
resp_test!(sunion, c, {
    let s1 = common::key("s1");
    let s2 = common::key("s2");
...
resp_test!(set_wrong_type_errors, c, {
    let k = common::key("wt");
